...
from Genetic_Algorithm.Population import Population, Individual, DenseUnit, ResUnit, PoolUnit
import logging
import sys
...
class GPUTools(object):
    ...
    def _get_available_gpu_plain_info(cls):
        ...
        lines_ = lines[:len(lines)-2]
        lines_.reverse()
        for line in lines_:
            if line.startswith('|==='):
                break
            else:
                gpu_info_list.append(line[1:-1].strip())
        # parse the information

        print('The use information of GPU:', gpu_info_list, sep='\n')
        ...
        if len(gpu_info_list) == 1:
            if gpu_info_list[0].startswith('No'):  # GPU outputs: No running processes found
                ...
                if info_array[0] == '0':
                    Log.info('GPU_QUERY-GPU#1 and #2 are available, choose GPU#1')
                    return 1
                elif info_array[0] == '1':
...
class Utils(object):
    ...
    def generate_pytorch_file(cls, indi):
        ...
        fully_layer_name = 'self.linear = nn.Linear(%d, %d)' % (
            image_output_size * out_channel_list[-1], StatusUpdateTool.get_num_class())
        ...
        for i, u in enumerate(indi.units):
            ...
            if u.type == 1:
                ...
                if u.max_or_avg < 0.5:
                    _str = 'out_%d = F.max_pool2d(out_%d, (1,2))' % (i, i - 1)
                else:
                    _str = 'out_%d = F.avg_pool2d(out_%d, (1,2))' % (i, i - 1)
                forward_list.append(_str)
...
if __name__ == '__main__':
    ...
    config = configparser.ConfigParser()
    config.read('Configuration.ini')
    q = config.get('settings', 'mutation_probs')
    print(type(q))
