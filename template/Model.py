...
class DenseNetBottleneck(nn.Module):
    def __init__(self, nChannels, growthRate):
        ...
        self.conv2 = nn.Conv2d(interChannels, growthRate, kernel_size=(1,3),
...
class TrainModel(object):
    def __init__(self):
        trainloader, validate_loader = Data_Loader_.get_train_valid_loader(r'/data_t.txt', batch_size=10, valid_size=0.2, shuffle=True, random_seed=2312390, show_sample=False, num_workers=1, pin_memory=True)
        #testloader = data_loader.get_test_loader('', batch_size=5, shuffle=False, num_workers=1, pin_memory=True)
        net = EvoCNNModel()
    ...
    def train(self, epoch):
        ...
        optimizer =optim.Adam(self.net.parameters(), lr=0.0001, weight_decay=5e-4)
        running_loss = 0.0
        total = 0
        correct = 0
        ...
        self.log_record('Train-Epoch:%3d,  Loss: %.3f, Acc:%.3f'% (epoch+1, running_loss/total, (correct.item()/total)))
    ...
    def test(self, epoch):
        ...
        for _, data in enumerate(self.validate_loader, 0):
            ...
            total += labels.size(0)
            correct += (predicted == labels).sum()
        if correct.item()/total > self.best_acc:
            self.best_acc = correct.item()/total
            #print('*'*100, self.best_acc)
        self.log_record('Validate-Loss:%.3f, Acc:%.3f%%'%(test_loss/total, self.best_acc*100))
